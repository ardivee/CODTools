...
namespace CODTools
{
    public class CODXAnimExport : MPxFileTranslator
    {
        ...
        public override void writer(MFileObject file, string optionsString, MPxFileTranslator.FileAccessMode mode)
        {
            ...
            {
                // Parse settings
                bool GrabNotes = true, ExportTagAlign = false;
                ...
                foreach (var Setting in SplitSettings)
                {
                    ...
                    else if (SettingValue[0] == "exportTagAlign")
                        ExportTagAlign = (SettingValue[1] == "1");
                }

                // Export anim
                CODXAnim.ExportXAnim(file.fullName, CODXAnim.XAnimType.Export, GrabNotes, ExportTagAlign);
            }
        }
    ...
    public class CODXAnimBin : MPxFileTranslator
    {
        ...
        public override void writer(MFileObject file, string optionsString, MPxFileTranslator.FileAccessMode mode)
        {
            ...
            if (file.fullName.ToUpper().EndsWith(".XANIM_BIN"))
            {
                ...
                // Export anim
                CODXAnim.ExportXAnim(file.fullName, CODXAnim.XAnimType.Bin, GrabNotes, ExportTagAlign);
            }
        }
