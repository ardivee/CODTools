...
namespace CODTools
{
    internal class CODXModel
    {
        ...
        public static void ExportXModel(string FilePath, XModelType FileType, bool Siege = false)
        {
            // Configure scene
            using (var MayaCfg = new MayaSceneConfigure())
            {
                ...
                // Sort joints
                SortJoints(ref Result, ParentStack);
        ...
        private static void SortJoints(ref XModel Model, List<string> ParentStack)
        {
            ...
            // Get all Cosmetic Bones
            foreach( var bone in Cosmetics.Bones )
            {
                var CosmeticBone = RootTree.Find(x => x.TagName == bone);

                if( CosmeticBone != null )
                {
                    CosmeticBone.isCosmetic = true;
                }
            }
